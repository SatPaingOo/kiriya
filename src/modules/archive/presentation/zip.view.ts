...
import type { ZipOutput } from "../application/create-zip.use-case.js";

export const zipView: TextView<ZipOutput> = (output, format) => {
  const ratio = output.bytesIn === 0 ? 100 : Math.round((output.bytesOut / output.bytesIn) * 100);
  const summary = message("archive.zip.done", {
  ...
  if (output.skippedLinks > 0) {
    lines.push(format.dim(format.text(message("archive.zip.skipped-links", { count: output.skippedLinks }))));
  }
  return lines;
};
