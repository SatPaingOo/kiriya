...
test("env show hides a secret-looking value unless --reveal is given", () => {
  const environment = { KIRIYA_NO_COLOR: "1", KIRIYA_TEST_API_TOKEN: "not-a-real-token" };
  const hidden = runKiriya(CWD, ["env", "show", "KIRIYA_TEST", "--json"], environment);
  assert.equal(hidden.code, 0, hidden.stderr);
  const { data } = JSON.parse(hidden.stdout) as { data: { variables: unknown[] } };
  assert.deepEqual(data.variables, [{ name: "KIRIYA_TEST_API_TOKEN", value: null, secret: true }]);
  assert.ok(!hidden.stdout.includes("not-a-real-token"));

  const shown = runKiriya(CWD, ["env", "show", "KIRIYA_TEST", "--reveal"], environment);
  assert.equal(shown.code, 0, shown.stderr);
  assert.match(shown.stdout, /^KIRIYA_TEST_API_TOKEN=not-a-real-token$/m);
});
