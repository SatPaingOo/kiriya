...
is done: `kiriya mcp` serves the `read` commands as MCP tools, and the `write` and
`destroy` ones when the user allows them, asking before any work that cannot be undone. The
...
| Exposure | `read` tools by default. `write` tools only when the user's config sets `mcp.allowWrite`. `destroy` tools only with `mcp.allowDestroy`, and each call still needs an elicitation the user accepts. Commands with `runsUserCommands` are never exposed, and neither are commands marked `terminalOnly`: `config set` and `config unset`, through which an agent could raise its own permissions or list a plugin. `docker up` and `docker rebuild` count as running user commands, since a compose file names the programs they start. For `write` tools the setting answers yes-or-no questions. Typed confirmations, in any tool, need `mcp.allowDestroy` and a client with elicitation, and are refused otherwise. No tool that changes something may reach kiriya's configuration file or anything inside a `.git` folder, and such tools do not offer options that walk into hidden folders. Commands and options marked `sensitive`, which can show secrets or send data away (`open`, `clip paste`, `proc list --full`), are offered only with `mcp.allowWrite`. |
| Confirmation | Elicitation in form mode with one string field for the typed value: the count or name, never a secret. Decline and cancel both end the call with nothing changed. A client without elicitation cannot run a `destroy` tool, and such tools are left out of the list when the request's client capabilities show no elicitation. Several popular clients lacked elicitation on 2026-09-13 ([RESEARCH.md](./RESEARCH.md) section 3). A `destroy` tool's yes-or-no questions go through elicitation too, as a form with no fields, so no call of it changes anything the user did not accept. In the 2026-07-28 version the question travels in an input-required result and the command runs again from the start when the client retries; the `requestState` names the tool, a digest of its arguments and the answers so far, lasts 15 minutes, and is signed with a secret of the process. Before 2026-07-28 the server sends `elicitation/create` itself and waits. A client without form elicitation that calls a `destroy` tool gets error -32021. |
...
| Publishing | When the repository is public: `mcpName` in `package.json`, a `server.json` in the MCP Registry, and an MCPB bundle for one-click install in Claude's desktop app. `mcpName` and `server.json` are ready; CI checks that they match `package.json`, and the release workflow publishes to the registry through GitHub OIDC after npm, with `mcp-publisher` pinned to a version and checked against its published checksum. Each release also attaches an MCPB bundle, which kiriya packs with its own `archive zip`; an end-to-end test builds it, unpacks it and starts its server on every operating system. |
...
| **3 — First release** | The v1 modules in section 5.2; `LICENSE` (MIT), `CHANGELOG.md`, `SECURITY.md`, `CODE_OF_CONDUCT.md`; then, once the maintainer judges it ready, the repository made public and the package published to npm from CI through trusted publishing with provenance | A clean machine on each OS installs kiriya from the README alone, `kiriya doctor` passes, and `npm audit signatures` verifies the package. **In progress 2026-09-14:** every v1 module, `LICENSE`, `CHANGELOG.md`, `SECURITY.md` and `CODE_OF_CONDUCT.md` are done, with CI green on all three operating systems; the npm name `kiriya` and the `@kiriya` scope were still free on 2026-09-13. The release workflow and its checks are ready ([docs/releasing.md](./docs/releasing.md)), and so is publishing to the MCP Registry; publishing waits for the maintainer. |
| **4 — AI** | `mcp` per section 6.8: `read` tools, then `write` tools behind `mcp.allowWrite`, then `destroy` tools behind `mcp.allowDestroy` and elicitation | A client built with the official SDK lists every tool with correct annotations, runs a `read` tool, is refused a path outside the roots, and cannot run a `destroy` tool without accepting an elicitation. **Done 2026-09-14:** `read`, `write` and `destroy` tools over both protocol versions, with roots checked through symlinks, cancellation and output limits. A client built with the official SDK, in both protocol eras, lists every tool with its annotations, runs a `read` and a `write` tool, is refused a path outside the roots, and deletes a file only after accepting the elicitation. CI green on Windows, Linux and macOS with Node 22, 24 and the current release. |
